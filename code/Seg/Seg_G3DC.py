...
def pretrain(**kwargs):
    ...
    for epoch in range(start_epoch, num_epochs):
        for data in train_loader:
            ...
            output = output.view(output.size(0), 1000)
...
def train(**kwargs):
    ...
    # optimizer1 = torch.optim.Adam(params=model.parameters(), lr=1e-4, weight_decay=1e-7)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        reconst_output = reconst_output.view(reconst_output.size(0), 1000)
        loss_mse = nn.MSELoss()(reconst_output, img)

        if epoch % 10 == 0:
            W1 = torch.tensor(parameters[0].detach().numpy(), requires_grad=True)
            np.savetxt('%s %s %s' % ('../model_weight/Seg_weight/layer_weight/W2', epoch, '.txt'), W1.cpu().detach().numpy())
    ...
    df.to_csv('../model_weight/Seg_weight/path/log_acc.csv')




if __name__ == '__main__':
    x, y, adj, gene_name = load_data()
    x = torch.tensor(x)
    y = torch.tensor(y)
    ...
    parser.add_argument('--batch_size', default=702, type=int)
    parser.add_argument('--pretrain_epochs', default=700, type=int)
    parser.add_argument('--train_epochs', default=220, type=int)
    parser.add_argument('--save_dir', default='../model_weight/Seg_weight/path')
    ...
    ae_save_path = '../model_weight/Seg_weight/path/sim_autoencoder.pth'
    ...
    dec_save_path = '../model_weight/Seg_weight/path/dec.pth'
